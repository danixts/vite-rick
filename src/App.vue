...
<template>
  ...
  <h1 class="font-bold text-5xl text-center text-white pt-[120px]">Rick and Morty</h1>
  <div class="container my-8">
    <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
      <Card :data="data" v-for="(data, index) in querySearch" :key="index" />
    </div>
    <template v-if="querySearch.length === 0">
      <h1 class="text-center text-white/25 mt-32 w-full text-3xl">Datos no encontrados</h1>
    </template>
  </div>
</template>
